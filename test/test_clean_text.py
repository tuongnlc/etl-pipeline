...
def clean_footer(text):
    # Biểu thức Regex tìm phần footer dựa trên cấu trúc của trang FILI
    # Giải thích mẫu: Tìm từ khóa FILI, sau đó là dấu gạch ngang và định dạng thời gian \d{2}:\d{2} \d{2}/\d{2}/\d{4}
    # [^\n]+ đại diện cho tên tác giả đứng ngay phía trên chữ FILI
    footer_pattern = r'[^\n]+\n+FILI\n+- \d{2}:\d{2} \d{2}/\d{2}/\d{4}'
    
    # Thay thế phần khớp bằng chuỗi rỗng và xóa khoảng trắng thừa ở cuối text
    cleaned_text = re.sub(footer_pattern, '', text)
    return cleaned_text.strip()

text_ = clean_footer(text_)
...
def clean_whitespace(text):
    ...
    # Bước 1: Thay thế tất cả các ký tự tab (\t) thành 1 khoảng trắng đơn
    text = re.sub(r'\t+', ' ', text)
    
    # Bước 4: Dọn dẹp khoảng trắng thừa ở đầu dòng và cuối dòng của toàn bộ văn bản
    return text

text_ = clean_whitespace(text_)


# remove unknow character
def clean_perfect_keep_blank_lines(text):
    if not text:
        return ""
    ...
    for line in text.split('\n'):
        # Nếu dòng đó hoàn toàn trống hoặc chỉ chứa khoảng trắng, giữ nguyên dòng trống đó
        if not line.strip():
            cleaned_lines.append("")
            continue
            
        # Nếu dòng có chữ, tiến hành lọc ký tự lạ:
        # Chỉ giữ lại chữ/số (\w) và khoảng trắng ngang ([ \t])
        processed_line = re.sub(r'[^\w \t]', ' ', line)
        
        # Gom các khoảng trắng liên tiếp trên dòng này thành 1 khoảng trắng đơn
        processed_line = re.sub(r'[ \t]+', ' ', processed_line)
        
        # Lưu dòng đã clean vào danh sách (bỏ khoảng trắng thừa ở đầu/cuối dòng)
        cleaned_lines.append(processed_line.strip())
        
    # Nối các dòng lại với nhau bằng dấu xuống dòng \n như ban đầu
    return '\n'.join(cleaned_lines)

text_ = clean_perfect_keep_blank_lines(text_)
# print(text_)
